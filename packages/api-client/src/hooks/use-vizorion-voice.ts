...
async function mockTranscribe(): Promise<{ text: string }> {
  return { text: '(mock transcription — set VITE_VIZORION_USE_MOCK=false for real speech-to-text)' }
}

const transcribeVoice = resolveVizorionService(mockTranscribe, vizorionClient.transcribeVoice)

export function useTranscribeVoice() {
  return useMutation({
    mutationFn: ({ audio, filename, language }: { audio: Blob; filename: string; language?: string }) =>
