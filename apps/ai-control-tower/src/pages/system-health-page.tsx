...
const INCIDENT_STATUS_VARIANT: Record<
  SystemIncident['status'],
...
function summarize(components: SystemComponentMetric[]) {
  ...
  const avgUptime =
    components.length === 0
      ? 0
      : Math.round(
          (components.reduce((sum, c) => sum + c.uptimePercent, 0) /
            components.length) *
            100,
        ) / 100
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Every infrastructure component the platform depends on, and its
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Current status of every backend service the platform depends on.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Rolling uptime and average response time per component.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {components.isLoading || !components.data ? (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Every incident logged against a platform component, resolved or not.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {incidents.data.map((incident) => (
                <li key={incident.id} className="text-sm">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge
                      variant={INCIDENT_STATUS_VARIANT[incident.status]}
